...
# Works on Debian/RPi OS; no-ops gracefully on Buildroot (no apt).
...
pkg_install openresolv || true  # fallback if resolvconf missing
...
if command -v systemctl >/dev/null 2>&1; then
    systemctl enable --now "wg-quick@$FIRST_WG" >/dev/null 2>&1 || {
...
if command -v resolvconf >/dev/null 2>&1; then
    ...
    log "[30_vpn] systemd-resolved present — DNS may be managed externally."
else
    log "[30_vpn] No resolvconf/systemd-resolved — ensure DNS handled inside tunnel or via NetworkManager."
fi
