...
probe_rtt_loss() {
  iface="$1"
  out="$(ping -I "$iface" -c 3 -w 4 -n "$PING_TARGET" 2>/dev/null || true)"
...
probe_throughput_mbit() {
  ...
  for url in "http://speed.hetzner.de/100MB.bin" "http://ipv4.download.thinkbroadband.com/100MB.zip"; do
    if has curl; then
      ...
      awk -v B="$bytes" -v T="$t" 'BEGIN{ if(T<=0){print 0; exit} print int((B*8)/(T*1000000)) }' && return 0
    fi
  done
  echo 0
}
...
main() {
  ...
  for cfg in /etc/wireguard/*.conf; do
    ...
    # temporarily bring up interface to test
    if [ "$cur" != "$name" ]; then
    ...
    [ "$s" -lt "$best_score" ] && { best="$name"; best_score="$s"; }
  done
